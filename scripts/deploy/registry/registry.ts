...
async function main() {
  ...
  const registry = await upgrades.deployProxy(
    registryFactory,
    [params.admins, params.tokenContract]
  );
  ...
  const proxyAdminAddress = await upgrades.erc1967.getAdminAddress(registry.address);
  const logic = await upgrades.erc1967.getImplementationAddress(registry.address);
  ...
  if (owner !== deployer.address) {
    ...
    if (owner !== (await registry.owner())) {
      ...
      let tx = await registry.transferOwnership(owner);
      await tx.wait();
    }
  ...
  for(let i = 0; i < params.admins.length; i++) {
    ...
    const isAdmin = await registry.isAdmin(admin);
  ...
  mustMatchAddress('Token', params.tokenContract, await registry.tokenContract());
  mustMatchAddress('RegistryOwner', owner, await registry.owner());
  mustMatchAddress('ProxyAdminOwner', owner, await upgrades.erc1967.getAdminAddress(registry.address));
