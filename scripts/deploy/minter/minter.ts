...
const mustMatchRatio = async (minter: Contract, expected: MinterRatio): Promise<void> => {
  ...
  if (!expected.numerator.eq(num)) {
    throw Error(`Expected numerator to be eq ${expected.numerator.toString()}, got ${num.toString()}`)
  }
  if (!expected.denominator.eq(den)) {
    throw Error(`Expected denominator to be eq ${expected.denominator.toString()}, got ${den.toString()}`)
  }
}
