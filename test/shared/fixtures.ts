...
const { abi: AdminRoleMockABI, bytecode: AdminRoleMockBytecode } =
  AdminRoleMock__factory;
const { abi: RegistryABI, bytecode: RegistryBytecode } = Registry__factory;
const { abi: MintableMockABI, bytecode: MintableMockBytecode } =
  MintableMock__factory;
const { abi: ERC20MockABI, bytecode: ERC20MockBytecode } = ERC20Mock__factory;
const { abi: MinterMockABI, bytecode: MinterMockBytecode } =
  NoopMinterMock__factory;
...
export const adminRoleMockFixture: Fixture<AdminRoleMockFixture> = async ([
  wallet,
]) => {
  const actors = new ActorFixture(provider.getWallets(), provider);
  const admins = [actors.adminFirst().address, actors.adminSecond().address];
  const adminRole = (await waffle.deployContract(wallet, {
...
export const tokenFixture: Fixture<TokenFixture> = async ([wallet]) => {
  ...
  const token = (await waffle.deployContract(wallet, {
...
export const registryFixture: Fixture<RegistryFixture> = async ([wallet]) => {
  const registerContributors = async (
  ...
  const registry = (await waffle.deployContract(wallet, {
