...
describe("unit/Registry", () => {
  ...
  describe("#registerContributor", () => {
    ...
    let subject: (_contributor: Contributor, _sender: Wallet) => Promise<any>;
    ...
    beforeEach(() => {
      ...
      check = async (_addr: Wallet | string) =>
        await context.registry.getMaxTrust(
          typeof _addr === "string" ? _addr : _addr.address
        );
    });
    ...
    describe("fails when", () => {
      ...
      it("trying to register address zero", async () => {
        await expect(
          subject(
            { account: AddressZero, maxTrust: 1000, balance: 1000 },
            actors.adminFirst()
          )
        ).to.be.reverted;
      });
      ...
      it("trying to set max trust to zero", async () => {
        await expect(
          subject(
            { account: actors.anyone().address, maxTrust: 0, balance: 1000 },
            actors.adminFirst()
          )
        ).to.be.reverted;
      });
      ...
      it("trying to register an existing contributor", async () => {
        ...
        await expect(
          subject(
            { account: existing, maxTrust: 1000, balance: 1000 },
            actors.adminFirst()
          )
        ).to.be.reverted;
      });
  ...
  describe("#removeContributor", () => {
    ...
    beforeEach(() => {
      subject = async (_addr: Wallet | string, _sender: Wallet) =>
        await context.registry
          .connect(_sender)
          .removeContributor(typeof _addr === "string" ? _addr : _addr.address);
      check = async (_addr: Wallet | string) =>
        await context.registry.getMaxTrust(
          typeof _addr === "string" ? _addr : _addr.address
        );
    ...
    describe("fails when", () => {
      ...
      it("trying to remove a non-contributor", async () => {
        await expect(subject(actors.anyone(), actors.adminFirst())).to.be
          .reverted;
      });
    });
  });
  ...
  describe("#registerContributors", () => {
    ...
    before(() => {
      subject = async (_contributors: Contributor[], _sender: Wallet) =>
        await context.registry
          .connect(_sender)
          .registerContributors(_contributors);

      check = async (_account: string) =>
        await context.registry.getMaxTrust(_account);
  ...
  describe("#getContributors", () => {
    ...
    describe("works and", () => {
      it("returns all contributors", async () => {
        expect(await subject()).to.have.same.members(
          context.state.everyone.map((c) => c.account)
        );
      });
    });
  });
  ...
  describe("#getMaxTrust", () => {
    ...
    beforeEach(() => {
      subject = async (_address: Wallet | string) =>
        await context.registry.getMaxTrust(
          typeof _address === "string" ? _address : _address.address
        );
    });
  ...
  describe("#getPendingBalance", () => {
    ...
    beforeEach(() => {
      subject = async (_adr: Wallet | string) =>
        await context.registry.getPendingBalance(
          typeof _adr === "string" ? _adr : _adr.address
        );
    });
